...
const layoutFitApi = (element, type, options) => {
    if (options)
        optionsApi(element, options);
...
class DOMRenderer {
    ...
    createElement(node) {
        const { tagname, styles, textContent, children, id, type, options, attributes } = node;
        ...
        if (type)
            layoutFitApi(element, type, options || "");
        if (children) {
